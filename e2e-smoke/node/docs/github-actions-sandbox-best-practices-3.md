# GitHub Actions Sandbox Best Practices (Research Stress 3)

This note condenses current GitHub primary-source guidance into a practical sandbox baseline for workflows that build code, process pull requests, or trigger privileged follow-up automation. The repeated advice across GitHub Docs is consistent: minimize `GITHUB_TOKEN` permissions, keep untrusted code away from privileged triggers, prefer short-lived federated credentials, pin actions immutably, and treat runners, caches, artifacts, logs, and downstream workflows as part of the trust boundary.

## Current Best-Practice Summary

- Declare explicit `permissions` and keep them narrow. GitHub documents that when you specify any permission, all unspecified permissions default to `none`.[1][2]
- Run contributor-controlled code on `pull_request`, not `pull_request_target`. GitHub warns that `pull_request_target` can expose secrets, write permissions, and cache state if you mix it with untrusted code from forks or attacker-controlled branches.[3][4]
- Treat `workflow_run` as privileged follow-up automation. GitHub notes that downstream workflows triggered by `workflow_run` can access secrets and write tokens even if the upstream workflow could not, so artifacts, caches, and outputs from low-trust runs must not be consumed blindly.[3][4]
- Prefer GitHub-hosted runners for workflows reachable by outside contributors. GitHub-hosted runners start from a fresh environment for each job, while self-hosted runners do not inherently provide a clean instance per run.[4][5][6]
...
- Pin third-party actions to full commit SHAs. GitHub documents a full-length SHA as the only immutable action reference and provides repository or organization controls to enforce SHA pinning.[4][8]
- Treat workflow inputs and GitHub context values as untrusted data. GitHub explicitly documents script injection risk from attacker-controlled fields such as branch names, pull request titles, issue bodies, and commit messages.[4][9]
- Keep logs, caches, and artifacts tight. GitHub recommends masking sensitive runtime values, setting explicit retention, and avoiding uploads that contain credentials, raw environment snapshots, or unnecessary debug data.[4][10]
- Use environments, required reviewers, and policy settings to protect high-value deployments and narrow the allowed actions surface where governance matters.[8][11][12]
...
- Treat PR titles, PR bodies, issue text, branch names, commit messages, workflow inputs, checked-out files, and downloaded artifacts as untrusted when constructing shell commands or generated scripts.[4][9]
...
- Pin external actions to full SHAs and verify that the pinned commit belongs to the intended upstream repository.[4][8]
- Restrict which actions and reusable workflows can run through repository, organization, or enterprise policy when you need stronger supply-chain controls.[8][12]
- Require review or approval for fork PR workflows according to your threat model, and inspect workflow-file changes carefully before approving outside runs.[4][11]
- Mask generated secrets and other sensitive runtime values with `::add-mask::` when they are not stored in GitHub Secrets.[4][10]
...
- Do pin third-party actions to verified full SHAs and periodically review their source and update path.[4][8]
- Do use environments, required reviewers, and repository or organization policy to add friction around sensitive deployments or dangerous action sources.[8][11][12]
- Do mask runtime secrets and keep artifact retention to the minimum practical window.[4][10]
- Do generate attestations for release artifacts that downstream users or admission policies are expected to verify.[13][14]
...
- Don't trust branch names, issue text, PR descriptions, or workflow inputs inside shell scripts without defensive handling.[4][9]
...
- Don't upload secrets, credential files, raw environment snapshots, or unnecessary debug archives as artifacts.[4][10]
- Don't approve fork PR workflows casually, especially when the change set touches `.github/workflows/` or other execution paths.[4][11]
...
9. GitHub Docs, "Script injections"  
   https://docs.github.com/en/actions/concepts/security/script-injections
10. GitHub Docs, "Use secrets"  
    https://docs.github.com/en/actions/how-tos/write-workflows/choose-what-workflows-do/use-secrets
11. GitHub Docs, "Approve runs from forks"  
    https://docs.github.com/en/actions/how-tos/manage-workflow-runs/approve-runs-from-forks
12. GitHub Docs, "Disabling or limiting GitHub Actions for your organization"  
