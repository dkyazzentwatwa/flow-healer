...
class Nalgo:
    ...
    def algoStart(self):
        ...
        time.sleep(1)
        choice = input(str("Choose operation: 'simple' , 'mega_scan', 'single_algo', 'info', 'tframe', 'symbol', 'ohlcv', 'exchange', 'clear' or 'exit'\n "))
        if choice == 'simple':
            print("Selected (simple)")
            bot.algo_single_scan()
        elif choice == 'mega_scan':
            print('Selected (mega_scan)')
            bot.algo_scan()
        elif choice == 'single_algo':
            print("Selected (single_algo)")
            bot.singleAlgo()
        elif choice == 'info':
            print("Selected (info)..")
            bot.currentInfo()
    ...
    def currentInfo(self):
        ...
        global scoreTotal
        global algoStore
        print("Exchange:", xchangename, "Symbol:", symbols, "Timeframe:", tframe, "ScoreTotal:", scoreTotal, "Close: ", close[99])
    ...
    def tframeChange(self):
        ...
        global sleeptime
        global scoreTotal
        tframe = input(str("Please enter a timeframe for OHLVC data; 30s, 1m, 5m, 15m, 30m, 1h, 2h, 3h, 4h, 6h, 1d, 1w, 1m"))
        if tframe == "1m" :
            sleeptime = int(60)
        elif tframe == '30s':
            tframe = "1m"
            ...
            sleeptime = int(604800)
        else:
            pass
        print("New timeframe is now:", tframe)
        print("New sleep timer is:", sleeptime , "seconds")
        scoreTotal = 0
        bot = Nalgo(xchangename, symbols, exchangeccxt, tframe)
        bot.algoStart()
    ...
    def algo_scan(self): 
        ...
        scoreTotal = 0 #reset score for next iteration
        ex_list = []
        fileName = "SCAN-" + self.xchangename + "-" + coin + "-" + tframe + "-" + nowTime + ".csv" #unique file name
        #for JSON storage
        ...
        bin_list = ["BTC/USDT", "ETH/USDT", "ADA/USDT", "BNB/USDT", "XRP/USDT", "SOL/USDT", "DOT/USDT", "DOGE/USDT", "UNI/USDT", "LTC/USDT", "SHIB/USDT", "FTM/USDT", "AVAX/USDT", "LUNA/USDT", "ALPINE/USDT", "WAVES/USDT", "NEAR/USDT", "TRX/USDT", "LINK/USDT", "MANA/USDT", "MATIC/USDT"]
        cbp_list = ["BTC/USD", "ETH/USD", "ADA/USD", "MATIC/USD", "MANA/USD", "SOL/USD", "DOT/USD", "DOGE/USD", "LINK/USD", "LTC/USD", "ALGO/USD", "AVAX/USD", "ATOM/USD", "XLM/USD", "OMG/USD"]        
        ftx_list = ["BTC/USD", "ETH/USD", "BNB/USD", "XRP/USD", "FTT/USD", "FTM/USD", "MATIC/USD", "LTC/USD", "SOL/USD"]
        ku_list = ["BTC/USDT", "ETH/USDT", "LUNA/USDT", "XRP/USDT", "MIR/USDT", "FTM/USDT", "AVAX/USDT", "JASMY/USDT", "SOL/USDT", "NEAR/USDT", "RUNE/USDT", "VRA/USDT", "ATOM/USDT", "WAVES/USDT"]
        kra_list = ["BTC/USD", "ETH/USD", "ADA/USD", "XRP/USD", "XRP/USD", "SOL/USD", "DOT/USD", "DOGE/USD", "UNI/USD"]
        huo_list = ["BTC/USDT", "ETH/USDT", "LUNA/USDT", "XRP/USDT", "TRX/USDT", "NFT/USDT", "NEAR/USDT", "SOL/USDT", "MATIC/USDT", "GALA/USDT"]
        up_list = ["BTC/KRW", "ETH/KRW", "STX/KRW", "ICX/KRW", "XRP/KRW", "SOL/KRW", "NEAR/KRW", "SAND/KRW", "HUM/KRW", "HUM/KRW", "GAS/KRW", "AVAX/KRW"]
        if xchangename == "binance":
            ex_list = bin_list
        if xchangename == "coinbasepro":
            ex_list = cbp_list
        if xchangename == "ftx":
            ex_list = ftx_list
        if xchangename == "kucoin":
            ex_list = ku_list

        if xchangename == "kraken":
            ex_list = kra_list
        if xchangename == "huobipro":
            ex_list = huo_list
        if xchangename == "upbit":
            ex_list = up_list
        ...
        for i in range(int(cycle)): 
            for coin in ex_list:
                bot = Nalgo(xchangename, coin, exchangeccxt, tframe)
                #nowTime = time.asctime().replace(" ", "")
                ...
                try:
                        ...
                        if scoreTotal >= 7:
                            ...
                            tradeSignal = "Downtrend || Don't Buy"
                            tradeTrigger = False
                        elif scoreTotal <= 0 :
                            tradeSignal = "Downtrend || Don't Buy"
                            tradeTrigger = False
                        ...
                        time.sleep(1)     
                        """
                        if Algo.objects(exchange=self.xchangename, symbol=coin).count() > 0: #checks if there are documents for current symbol & exchange
                            update1 = Algo.objects(symbol=coin, exchange=self.xchangename)
                            ...
                            print("Updating old documents for ", coin, "on", self.xchangename)
                        else:
                            results = Algo(symbol=coin, exchange=self.xchangename, tframe=self.tframe, tradeSignal=tradeSignal, scoreTotal=scoreTotal, high=high[99], low=low[99], close=close[99])
                            results.save()
                            timeUpdate = Algo.objects(timeName=timedName)
                            timeUpdate.update(set__currentTime=theTime)
                            print("Created new document for ", coin, "on", self.xchangename)
                            pass
                        """
    ...
    def algo_single_scan(self): 
        ...
        global symbol
        global xchangename
        bot = Nalgo(xchangename, self.symbols, exchangeccxt, tframe)
        scoreTotal = 0 #reset score for next iteration
        nowTime = time.asctime().replace(" ", "")   
...
def handle_new_bar(data, emit, atoms):
    #!BLOCKS
    if data.RSI() >= 65:
        if data.EMA(ln=20) > data.EMA(ln=200):
            if data.SMA(ln=20) < data.SMA(ln=200):
                emit.Buy(unit="max", reason=" == 1")
    else:
        pass
    if data.RSI() < 37:
        if data.EMA(ln=20) < data.EMA(ln=200):
            if data.SMA(ln=20) < data.SMA(ln=200):
                emit.Sell(unit="max", reason="")
    else:
        pass
