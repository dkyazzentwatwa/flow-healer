...
xchange = exchangeccxt(exchangeinfo) #binds api keys to ccxt module
symbol = input(str("Enter Coin Pair (ABC/DFG):  ")) #ABC/DFG..ABC = Base Coin; DFG = Quote Coin
...
class NobiBot:
    ...
    def analyze(self): #begin pinging of stats like:OHLVC, bid/ask, etc. this gets the bot ready w/ data// 
        ...
        for i in range(int(cycle)):
            ...
            #bchbtc_binance_df = pd.DataFrame(bchbtc_binance_ob).drop(columns = ["datetime","timestamp","nonce"])[:1]
            print(prices_df)
...
#ARIGATO function is the main driver, it handles the conditions & everything always loops back to here, then back to trading..
    def arigato(self):
        ...
        #completed_trades = 0
        #trading toggling condtions (should be universal to any algorithm)
        while True:
...
#BUY 
    def trade(self):
        ...
        while sell_symbol_id in str(xchange.fetchOpenOrders(symbol)):
            ...
            #profit_log.append(order_output)
...
#Each is a variant of trade()? Each indicator will have to be awaited and constantly pinged...
