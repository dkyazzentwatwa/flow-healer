...
async def on_message(message):
    ...
    if any(command in message.content.lower() for command in commands_list):
        ...
        await message.author.send(msg + ', currently the only commands that are active are !price "Insert Ticker Here" which will display the current price of a stock.')
