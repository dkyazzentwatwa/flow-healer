...
# --- NOBIBOT LIVE-TRADE VERSION 2.0 ---
...
#Initiation
xchangename = ""
mainkey = os.getenv('EXCHANGE_API_KEY', '')
...
# Paper trading mode (set in .env)
PAPER_TRADING = os.getenv('PAPER_TRADING', 'true').lower() == 'true'
exchangeccxt = "" #binds xchangename to ccxt module
xchange = "" #binds api keys to ccxt modulein Pair (ABC/DFG):  ")) #ABC/DFG..ABC = Base Coin; DFG = Quote Coin
coin1 = "" #Quote Coin (BASE/QUOTE) (BNB/BTC)
coin1_balance = "" #For Test uses static amount; Quote
coin2 = ""#Base Coin (BASE/QUOTE) (BNB/BTC)
coin2_balance = "" #For Test uses static amount; Base
startamount = 0 #amount to start trade, balance cannot go udner this amount
startamount2 = 0
stop_amount = 0
stop_amount2 = 0
trade_amount = 0
...
class Nalgo:
    ...
    def algoStart(self):
        ...
        time.sleep(1)
        choice = input(str("""Choose operation: 'scan', ohlcv', 'live-trade', 'test-trade', 'info', 'tframe', 'symbol', \n 
                            'change-exchange', 'balance', 'clear' or 'exit'\n """))
        if choice == 'scan':
            print("Selected (scan)")
            bot.algo_scan()
        elif choice == 'live-trade':
            print("Selected (trade)")
            bot.tradeInit()
        elif choice == 'info':
            print("Selected (info)")
            bot.currentInfo()
            ...
            bot.quickOHLCV()
        elif choice == 'change-exchange':
            print("Selected (exchange)")
            bot.newExchange()
        elif choice == 'balance':
            print("Selected (balance) ")
            bot.balance()
        elif choice == 'clear':
    ...
    def balance(self): #Balance
        print("Current balance is: ", xchange.fetchBalance() )
        time.sleep(1)
    ...
    def fetchorders(self):
        print("Current orders are: ", xchange.fetch_my_trades() )
        bot = Nalgo(xchangename, symbols, exchangeccxt, tframe)
        bot.algoStart()
    ...
    def newExchange(self):
        ...
        """
        trading = input(str("Are you trading? If 'yes', enter API keys now~))
        if trading == "yes"
        else:
            bot = Nalgo(xchangename, symbols, exchangeccxt, tframe)
            bot.algoStart()
    ...
    def quickOHLCV(self):
        ...
        print("Listing last", limits, " OHLCV points for the tframe:", tframe, "for:", symbols)
    ...
    def symbolChange(self):
        ...
        print("New symbol pair is:", symbols, "Coin1:", coin1, "Coin2:", coin2)
    ...
    def tradeInit(self):   
        ...
        try:
            ...
            mainkey = os.getenv('EXCHANGE_API_KEY') or input(str("Enter your MAIN API key: "))
            secretkey = os.getenv('EXCHANGE_SECRET_KEY') or input(str("Enter your SECRET API key: "))
            exchangeinfo = {
                'apiKey': mainkey,
                'secret': secretkey,
                'enableRateLimit': True,
            ...
            fileName = "TRADE-" + self.xchangename + "-" + self.symbols + "-" + self.tframe + "-" + nowTime + ".csv" #unique file name
            filename2 = fileName.replace("/", "")
            tradeFile = filename2.replace(":", "")
            tradeIndexNumber = 0
    ...
    def arigato(self):
        ...
        try:
            while True:
                len(self.xchange.fetchOpenOrders(self.symbols)) == 0
                if open_order > 1: #security to make sure there are not too many orders, and waits for sales #7
                    ...
                    Order_Status = True
                    open_orders = self.xchange.fetchOpenOrders(self.symbols)
                    if open_orders:
                        order_side = open_orders[0].get('side', '').lower()
                        order_id = open_orders[0].get('id', 'N/A')
                        print(f'Order for {symbols} is active. Order Status: {Order_Status}, ID: {order_id}')
                        if order_side == 'sell':
                            print(f'Order for {symbols} waiting to be sold...')
                            time.sleep(2)
                            bot.arigato()
                        elif order_side == 'buy':
                            print(f'Order for {symbols} waiting to be bought...')
                            time.sleep(2)
                            bot.arigato()
                    else:
                        pass
    ...
    def trade(self):
        ...
        fileName = "Live_Trade" + self.xchangename + "-" + self.symbols + "-" + self.tframe + "-" +  nowTime + ".csv" #unique file name
        filename2 = fileName.replace("/", "")
        ...
        try:
            ...
            while tradeTrigger == False:
                ...
                self.xchange.create_limit_buy_order(self.symbols, buyamount, buyprice)
                time.sleep(2)
                while len(self.xchange.fetchOpenOrders(self.symbols)) == 0:
                    ...
                    buy_symbol_id =  self.xchange.fetchOpenOrders(self.symbols)[0]['id'] #'1234567890' Static for test
        ...
        try:
            while tradeTrigger == True:
                ...
                while len(self.xchange.fetchOpenOrders(self.symbols)) == 0:
                    ...
                    sell_symbol_id = self.xchange.fetchOpenOrders(self.symbols)[0]['id'] #'987654321'
                time.sleep(1)
                ...
                print('Order for ', symbols, sell_symbol_id, 'has been sold for: ', sell_amount, self.coin2, 'Profit:',trade_profit, 'Coin1 Balance:', xchange.fetchBalance()[self.coin1], coin1, "And Coin2 Balance: ", xchange.fetchBalance()[self.coin2], coin2, "Profit: ", totalProfit, 'Returning to main loop...')
                #fees = 
                open_order = 0
                closed_orders += 1
    ...
    def algo_scan(self): 
        ...
        try: #for error handling
            for i in range(int(cycle)): 
                ...
                print("Next analysis will be in ", self.sleeptime , "seconds. Press Control+C to exit to menu.")
                time.sleep(self.sleeptime)
        except ccxt.NetworkError as e:
            print('network error:', str(e))
        # retry or whatever
...
if __name__ == '__main__':
    ...
    # Use environment variables if available, otherwise prompt
    xchangename = os.getenv('DEFAULT_EXCHANGE') or input(str("Please enter exchange name (binance, binanceus, kraken, etc.):  "))
    mainkey = os.getenv('EXCHANGE_API_KEY') or input(str("Enter your MAIN API key: "))
    secretkey = os.getenv('EXCHANGE_SECRET_KEY') or input(str("Enter your SECRET API key: "))
    ...
    exchangeccxt = getattr(ccxt, xchangename) #This binds 'xchangename' to ccxt module.
    xchange = exchangeccxt(exchangeinfo) #This binds api keys to ccxt module. 
    """
    A base currency (a transaction currency) is the first currency that appears in a currency pair quotation. 
    Consequently, the second currency in the pair is a quote currency (a counter currency).
    In quotation, BTC/USDT 8,243.35, means the base currency 1 BTC is traded against 
    the quote currency USDT and exchanged for 8,243.35 USDT.
    """
    coin1 = input(str("Please Enter Coin1(Base coin) (BASE/QUOTE) :  \n")) #Base Coin (BASE/QUOTE) (BNB/BTC)
    coin2 = input(str("Please Enter Coin2(Quote coin) (BASE/QUOTE) :  \n")) #Quote Coin (BASE/QUOTE) (BNB/BTC)
    time.sleep(2)
    print("Current balance is: ", xchange.fetchBalance() )
    time.sleep(1)
    print("Current orders are: ", xchange.fetch_my_trades() )
    time.sleep(1)
    #xchange.fetch_balance()
    #coin1_balance = xchange.fetchBalance()[coin1] #For Test uses static amount; Base
    #coin2_balance = xchange.fetchBalance()[coin2] #For Test uses static amount; Quote
    symbols = input(str("Please Enter Coin Pair (ex. BTC/USDT):  \n")) #ex. ABC/DFG..A BC = Base Coin; DFG = Quote Coin
    tframe = input(str("Please enter a timeframe for OHLVC data; 30s, 1m, 5m, 15m, 30m, 1h, 2h, 3h, 4h, 1d, 1w, 1m  \n"))
    if tframe == "1m" :
