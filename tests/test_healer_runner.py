...
def test_stage_workspace_changes_excludes_rust_lockfile_when_only_validation_mentions_cargo(tmp_path):
    workspace = tmp_path / "repo"
    workspace.mkdir()
    _init_git_repo(workspace)
    (workspace / "src").mkdir()
    (workspace / "src" / "lib.rs").write_text("pub fn add(left: i32, right: i32) -> i32 { left - right }\n", encoding="utf-8")
    subprocess.run(["git", "add", "."], cwd=workspace, check=True, capture_output=True, text=True)
    subprocess.run(["git", "commit", "-m", "init"], cwd=workspace, check=True, capture_output=True, text=True)

    (workspace / "src" / "lib.rs").write_text("pub fn add(left: i32, right: i32) -> i32 { left + right }\n", encoding="utf-8")
    (workspace / "Cargo.lock").write_text("# generated by cargo\n", encoding="utf-8")

    changed = _stage_workspace_changes(
        workspace,
        issue_title="Rust smoke fix",
        issue_body="Validation:\n- cd e2e-smoke/rust && cargo test\n",
        task_spec=HealerTaskSpec(
            task_kind="fix",
            output_mode="patch",
            output_targets=("src/lib.rs",),
            tool_policy="repo_only",
            validation_profile="code_change",
            language="rust",
        ),
    ...
    assert changed is True
    assert _changed_paths(workspace) == ["src/lib.rs"]


def test_stage_workspace_changes_excludes_rust_target_artifacts_when_only_validation_mentions_cargo(tmp_path):
    workspace = tmp_path / "repo"
    workspace.mkdir()
    _init_git_repo(workspace)
    (workspace / "src").mkdir()
    ...
    (workspace / "target" / "debug" / "demo").write_text("compiled\n", encoding="utf-8")
...
def test_run_attempt_recleans_regenerated_rust_lockfile_contamination(monkeypatch, tmp_path):
    workspace = tmp_path / "repo"
    workspace.mkdir()
    _init_git_repo(workspace)
    ...
    def fake_validate_workspace(*args, **kwargs):
        ...
        (workspace / "Cargo.lock").write_text(
            f"# regenerated by cargo {validate_calls['count']}\n",
            encoding="utf-8",
        )
        (workspace / "target" / "debug").mkdir(parents=True, exist_ok=True)
        (workspace / "target" / "debug" / f"demo-{validate_calls['count']}").write_text(
            "compiled\n",
            encoding="utf-8",
        )
        return {
    ...
    result = runner.run_attempt(
        issue_id="lockfix-rust-1",
        issue_title="Fix rust add helper",
        issue_body="Update src/lib.rs only.",
        task_spec=HealerTaskSpec(
            task_kind="fix",
            output_mode="patch",
            output_targets=(),
    ...
    assert result.failure_class == ""
    assert result.workspace_status["cleanup_cycles_used"] >= 1
    assert "Cargo.lock" in result.workspace_status["cleaned_paths"]
    assert any(path.startswith("target/") for path in result.workspace_status["cleaned_paths"])
    assert validate_calls["count"] >= 2
    assert "Cargo.lock" not in _changed_paths(workspace)
    assert not any(path.startswith("target/") for path in _changed_paths(workspace))
...
def test_run_attempt_prefers_boot_result_readiness_url_for_browser_entry_url(tmp_path):
    workspace = tmp_path / "repo"
    workspace.mkdir()
    _init_git_repo(workspace)
    (workspace / "demo.py").write_text("def add(a, b):\n    return a - b\n", encoding="utf-8")
    ...
    class _PortShiftAppHarness(_FakeAppHarness):
        def boot(self, profile):
            self.boot_calls.append(profile)
            session = _FakeAppHarnessSession(profile)
            self.sessions.append(session)
    ...
    app_harness = _PortShiftAppHarness()
    browser_harness = _FakeBrowserHarness(
        [
            BrowserJourneyResult(
                phase="failure",
                passed=False,
                expected_failure_observed=True,
                final_url="http://localhost:3001/login",
                failure_step="expect_text Artifact Proof A1",
                error="Artifact Proof A1",
                screenshot_path=str(tmp_path / "before.png"),
                video_path=str(tmp_path / "before.webm"),
                console_log_path=str(tmp_path / "before-console.log"),
                network_log_path=str(tmp_path / "before-network.jsonl"),
                transcript=({"step": "goto /login", "status": "passed"},),
            ),
            BrowserJourneyResult(
                phase="failure-replay",
                passed=False,
    ...
    result = runner.run_attempt(
        issue_id="app-shifted-runtime-url",
        issue_title="Fix browser-backed regression",
        issue_body="Fix demo.py",
        task_spec=HealerTaskSpec(
            task_kind="fix",
            ...
            entry_url="/login",
            runtime_profile="web",
            repro_steps=("goto /login", "expect_text Artifact Proof A1"),
            artifact_requirements=("screenshot: artifacts/demo.png",),
        ),
        workspace=workspace,
        max_diff_files=5,
    ...
    assert result.test_summary["runtime_summary"]["app_harness"]["readiness_url"] == "http://localhost:3001/"
    assert result.test_summary["runtime_summary"]["app_harness"]["entry_url"] == "http://localhost:3001/login"
...
def test_run_attempt_blocks_promotion_when_declared_browser_logs_are_missing(tmp_path):
    workspace = tmp_path / "repo"
    workspace.mkdir()
    _init_git_repo(workspace)
    (workspace / "demo.py").write_text("def add(a, b):\n    return a + b\n", encoding="utf-8")
    ...
    result = runner.run_attempt(
        issue_id="app-constructive-logs-1",
        issue_title="Java browser artifact smoke",
        issue_body="Update demo.py",
        task_spec=HealerTaskSpec(
            task_kind="edit",
    ...
    assert "console" in result.failure_reason.lower()
...
def test_run_attempt_classifies_same_origin_asset_boot_failures_as_runtime_readiness(tmp_path):
    workspace = tmp_path / "repo"
    workspace.mkdir()
    _init_git_repo(workspace)
    (workspace / "demo.py").write_text("def add(a, b):\n    return a - b\n", encoding="utf-8")
    ...
    browser_harness = _FakeBrowserHarness(
        [
            ...
            BrowserJourneyResult(
                ...
                expected_failure_observed=False,
                final_url="http://127.0.0.1:3000/",
                failure_step="expect_text Winner: X",
                error="Winner never appeared",
                screenshot_path=str(tmp_path / "after.png"),
                video_path=str(tmp_path / "after.webm"),
                console_log_path=str(tmp_path / "after-console.log"),
                network_log_path=str(tmp_path / "after-network.jsonl"),
                transcript=({"step": "expect_text Winner: X", "status": "failed"},),
    ...
    result = runner.run_attempt(
        issue_id="app-205-runtime-ready",
        issue_title="Fix browser-backed regression",
        issue_body="Fix demo.py",
        task_spec=HealerTaskSpec(
            task_kind="fix",
