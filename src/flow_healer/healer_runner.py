...
def _build_retry_prompt(
    *,
    ...
    if failure_class in {"no_patch", "no_workspace_change", "empty_diff"}:
...
def _materialize_completion_artifact(
    *,
    ...
    This is a last-resort fallback: instead of hard-failing with no output, we produce a
    markdown paper trail in docs/healer-runs/ that records what the agent attempted. The
    file is excluded from generated-artifact filtering (docs/healer-runs/ is not in
    _GENERIC_ARTIFACT_DIRS) so it will be staged and committed normally.

    Returns True if the artifact was written (and caller should re-check staged changes).
    """
