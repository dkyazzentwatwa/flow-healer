...
class ClineConnector:
    """Stateless connector that shells out to `cline -y` for each turn."""
    ...
    def __init__(
        ...
        *,
        ...
        self.cline_command = (cline_command or "cline").strip() or "cline"
        self.timeout = timeout
        self.model = (model or "").strip()
        self.use_json = bool(use_json)
        self.act_mode = bool(act_mode)
    ...
    def ensure_started(self) -> None:
        ...
        try:
            ...
            with self._lock:
                self._available = False
                self._availability_reason = f"cline health check failed: {exc}"
        ...
        with self._lock:
            ...
            if proc.returncode == 0:
                ...
                output = (proc.stderr or proc.stdout or f"cline exited {proc.returncode}").strip()
                self._available = False
    ...
    def run_turn(self, thread_id: str, prompt: str, *, timeout_seconds: int | None = None) -> str:
        ...
        if not available or not resolved:
            return f"ConnectorUnavailable: {reason or 'Cline CLI command is not available.'}"
        ...
        cmd.extend(["-c", self.workspace, prompt])
        ...
        try:
            ...
            with self._lock:
                self._available = False
                self._availability_reason = f"Cline CLI not found at '{resolved}'."
                self._last_health_error = self._availability_reason[:500]
            return f"ConnectorUnavailable: Cline CLI not found at '{resolved}'."
        except subprocess.TimeoutExpired:
            return f"ConnectorRuntimeError: Cline CLI timed out after {int(effective_timeout)}s."
        except Exception as exc:
            return f"ConnectorRuntimeError: Cline CLI execution failed: {exc}"

        if proc.returncode != 0:
            output = (proc.stderr or proc.stdout or f"Cline CLI exited with code {proc.returncode}").strip()
            with self._lock:
                self._last_health_error = output[:500]
            return f"ConnectorRuntimeError: {output[:800]}"
        if self.use_json:
            parsed = _extract_cline_json_response(proc.stdout or "")
            if parsed:
                return parsed
    ...
    def health_snapshot(self) -> dict[str, str | bool]:
        with self._lock:
            return {
                "available": self._available,
                "configured_command": self.cline_command,
    ...
    def _resolve_command(self) -> None:
        ...
        with self._lock:
            if resolved:
                ...
                self._available = False
                self._availability_reason = (
                    f"Unable to resolve Cline command '{self.cline_command}'. "
                    "Set service.cline_command to an absolute path."
                )


def _extract_cline_json_response(raw_output: str) -> str:
    completion = ""
    fallback = ""
    for line in str(raw_output or "").splitlines():
        ...
        try:
            ...
            continue
        if obj.get("type") != "say" or obj.get("partial") is True:
            continue
