# GitHub Actions Sandbox Best Practices (Research Stress 16)

This note condenses current GitHub primary-source guidance into a practical sandbox baseline for CI, release automation, and contributor-triggered workflows. The recurring advice in GitHub Docs is consistent: keep `GITHUB_TOKEN` permissions minimal, avoid running untrusted code in privileged contexts, prefer short-lived federated credentials, pin third-party actions immutably, and treat runners, caches, logs, artifacts, and downstream workflows as part of the trust boundary.

## Current Best-Practice Summary

- Set explicit `permissions` on every workflow or job and keep them minimal. GitHub documents that once permissions are declared explicitly, unspecified scopes default to `none`.[1][2]
- Run contributor-controlled code on `pull_request`, not `pull_request_target`. GitHub warns that `pull_request_target` with untrusted code can expose secrets, write tokens, or poison caches.[3][4]
- Treat `workflow_run` as privileged follow-up automation. GitHub notes that downstream workflows triggered this way can access secrets and write tokens even if the upstream workflow could not, so untrusted artifacts and outputs must not flow into them without review.[3][4]
- Prefer GitHub-hosted runners for workflows influenced by forks or external contributors. GitHub-hosted runners start in a fresh environment per job, while self-hosted runners do not inherently provide a clean instance for every run.[4][5][6]
- Prefer OIDC federation over long-lived cloud credentials. Grant `id-token: write` only to the specific job that needs cloud identity and scope trust policies to the intended repository and execution context.[7]
- Pin third-party actions to full commit SHAs. GitHub documents a full-length SHA as the only immutable release reference for an action.[4][8][9]
- Treat workflow inputs and GitHub context values as untrusted. GitHub's secure-use guidance explicitly warns about script injection risks from attacker-controlled fields such as PR titles, branch names, and issue bodies.[4]
- Keep logs, caches, and artifacts tight. GitHub recommends masking sensitive values that are not stored as secrets, setting explicit artifact retention, and avoiding confidential uploads.[10][11]
- Use artifact attestations for release outputs and other high-value build artifacts. GitHub positions attestations as a supply-chain control for establishing and verifying build provenance, not as a blanket guarantee for every CI artifact.[12][13]
- Use repository, organization, or enterprise Actions policies to restrict allowed actions, require immutable pinning, and tighten fork and reusable-workflow behavior where governance matters.[8][9][14]
...
- Review `workflow_run` designs so an untrusted upstream job cannot pass poisoned artifacts, outputs, or cache state into a privileged downstream workflow.[3][4]
...
- If self-hosted runners are required, isolate them from sensitive networks, avoid long-lived credentials on the host, and prefer ephemeral or just-in-time runner patterns so each job starts clean.[4][6][14]
...
- Generate and verify artifact attestations for binaries, container images, packages, or other released artifacts that consumers should be able to validate.[12][13]
- Protect sensitive deployments with environments and approval controls when workflows can reach high-value secrets or production systems.[10][14]
...
- Do mask runtime secrets and shorten artifact retention to the minimum practical window.[10][11]
- Do generate attestations for release artifacts that downstream users or admission policies should verify.[12][13]
- Do use repository, org, or enterprise policy to limit allowed actions and require immutable pinning where available.[8][9][14]
...
- Don't treat `workflow_run` as harmless glue if it consumes artifacts or outputs from untrusted upstream workflows.[3][4]
...
- Don't assume artifact attestations alone make a build safe; GitHub documents them as provenance evidence that still requires verification policy and risk review.[12]
...
Use artifact attestations for released binaries, container images, packages, or manifests that consumers should verify. GitHub's guidance is that attestations help establish provenance, but they are most useful when paired with verification and policy at the point of consumption.[12][13]
