# GitHub Actions Sandbox Best Practices (Research Stress 11)

This note distills current GitHub primary-source guidance into a practical sandbox hardening baseline for repositories that run CI, deployment, or repository automation. The recurring themes in GitHub Docs are consistent: keep `GITHUB_TOKEN` permissions minimal, run untrusted code only in low-privilege contexts, prefer short-lived federated credentials over stored cloud keys, pin third-party actions immutably, and treat runners, artifacts, caches, logs, and downstream workflows as part of the trust boundary.

## Current Best-Practice Summary

- Declare explicit `permissions` and keep them narrow. GitHub documents that once any permission is specified, all unspecified permissions default to no access.[1][2]
- Run contributor-controlled code on `pull_request`, not `pull_request_target`. GitHub warns that `pull_request_target` can expose secrets, write permissions, or shared state if it is combined with untrusted code.[3][4]
- Treat `workflow_run` as a privilege boundary. GitHub notes that downstream workflows triggered this way can access secrets and write tokens even when the upstream workflow could not, so artifacts, outputs, or caches from untrusted runs need explicit review before reuse.[3][4]
- Prefer GitHub-hosted runners for workflows influenced by forks or outside contributors. GitHub-hosted runners start in a fresh environment for each job, while self-hosted runners do not inherently provide a clean instance per run.[4][5][6]
- Prefer OIDC federation over long-lived cloud credentials. Grant `id-token: write` only to the job that actually needs cloud identity and scope the trust policy to the intended repository and execution context.[7]
- Pin third-party actions to a full commit SHA. GitHub documents a full SHA as the only immutable action reference and supports policy controls that can require SHA pinning.[4][8][9]
- Treat workflow inputs and GitHub context values as untrusted data. GitHub explicitly warns about script injection risks from attacker-controlled fields such as PR titles, issue bodies, branch names, and commit messages.[4]
- Keep logs, caches, and artifacts tight. Mask sensitive runtime values, avoid uploading credentials or raw environment dumps, and set short retention for artifacts that must exist.[10][11]
- Use artifact attestations for released binaries, packages, or container images where provenance matters. GitHub positions attestations as a supply-chain control that works best when consumers also verify them.[12][13]
- Use repository, organization, or enterprise Actions policy to restrict allowed actions and reusable workflows when you need stronger supply-chain controls.[8][9][14]
...
- Move elevated permissions to the narrowest possible job instead of granting workflow-wide write access.[1][2]
- Use `pull_request` for build, test, lint, and any workflow that checks out or executes pull request code.[3]
- Reserve `pull_request_target` for metadata-only tasks such as labeling, commenting, or triage, and do not combine it with checkout of fork code, untrusted artifact download, or cache writes derived from attacker-controlled input.[3][4]
- Review `workflow_run` designs so an untrusted upstream run cannot pass poisoned artifacts, outputs, or caches into a privileged downstream workflow.[3][4]
- Treat PR titles, PR bodies, issue text, workflow inputs, branch names, commit messages, artifacts, and checked-out files as untrusted when constructing shell commands or generated scripts.[4]
- Prefer GitHub-hosted runners for public repositories and any workflow reachable by external contributors.[4][5][6]
- If self-hosted runners are required, isolate them from sensitive networks, avoid long-lived credentials on the host, and prefer ephemeral or just-in-time patterns so each job starts clean.[4][6]
...
- Restrict which actions and reusable workflows can run through repository, organization, or enterprise policy when you need stronger governance.[8][9][14]
...
- Generate and verify artifact attestations for released outputs that downstream users or admission controls should validate.[12][13]
...
- Do generate attestations for release artifacts that consumers or admission policies should verify.[12][13]
- Do use repository, organization, or enterprise policy to limit allowed actions and require immutable pinning where available.[8][9][14]
...
- Don't assume self-hosted runners are clean between jobs or safe for untrusted public PRs by default.[6][14]
