# GitHub Actions Sandbox Best Practices (Research Stress 9)

This note condenses current GitHub primary-source guidance into a practical sandbox baseline for workflows that run CI, process pull requests, or perform privileged follow-up automation. The recurring advice in GitHub Docs is consistent: keep `GITHUB_TOKEN` permissions minimal, avoid mixing untrusted code with privileged triggers, prefer short-lived federated credentials, pin external actions immutably, and treat runners, caches, artifacts, and logs as part of the trust boundary.

## Current Best-Practice Summary

- Set explicit `permissions` on each workflow or job and keep them minimal. GitHub documents that once permissions are declared explicitly, all unspecified permissions are set to no access except `metadata`, which remains read-only.[1][2]
- Run contributor-controlled code on `pull_request`, not `pull_request_target`. GitHub warns that running untrusted code on `pull_request_target` can lead to cache poisoning and unintended access to secrets or write privileges.[3][4]
- Treat `workflow_run` as privileged follow-up automation. GitHub notes that a downstream workflow triggered by `workflow_run` can access secrets and write tokens even if the upstream workflow could not, so artifacts, outputs, and caches from lower-trust runs must not be consumed blindly.[3][4]
- Prefer GitHub-hosted runners for workflows influenced by forks or external contributors. GitHub-hosted runners start in a fresh isolated VM per job, while self-hosted runners do not inherently guarantee an ephemeral clean environment.[4][5][6]
- Prefer OIDC federation over long-lived cloud credentials. Grant `id-token: write` only to the job that needs cloud identity and scope trust policies to the repository, ref, environment, or reusable-workflow context that should actually receive access.[7]
- Pin third-party actions to full commit SHAs. GitHub documents a full-length SHA as the only immutable action reference and supports repository or organization policy to require that pinning mode.[4][8][9]
- Treat workflow inputs and GitHub context values as untrusted data. GitHub explicitly warns about script injection through attacker-controlled values such as issue bodies, PR titles, branch names, commit messages, and other interpolated context fields.[4]
- Keep logs, caches, and artifacts tight. GitHub recommends masking runtime secrets that are not stored as GitHub secrets, setting explicit artifact retention, and avoiding uploads that contain credentials, environment dumps, or unnecessary debug data.[10][11]
- Use artifact attestations for released binaries, packages, container images, or other high-value build outputs. GitHub positions attestations as provenance evidence that becomes useful when consumers verify it, not as a blanket guarantee that every artifact is safe.[12][13]
- Use repository, organization, or enterprise Actions policies to restrict which actions and reusable workflows can run, whether actions can create or approve pull requests, and when immutable pinning is required.[8][9][14]
...
- If self-hosted runners are required, isolate them from sensitive networks, avoid long-lived credentials on the host, and prefer ephemeral or just-in-time runners so each job starts clean.[4][6][14]
...
- Keep the repository-level `GITHUB_TOKEN` default restrictive and leave "Allow GitHub Actions to create and approve pull requests" disabled unless a reviewed workflow genuinely needs that capability.[8][14]
...
- Generate and verify artifact attestations for released binaries, container images, packages, or manifests that downstream users should validate.[12][13]
...
- Do use repository, organization, or enterprise policy to narrow the allowed action surface and require immutable pinning where available.[8][9][14]
...
- Don't assume self-hosted runners are clean between jobs or safe for untrusted public PRs by default.[4][6][14]
...
14. GitHub Docs, "Managing GitHub Actions settings for a repository"  
    https://docs.github.com/en/repositories/managing-your-repositorys-settings-and-features/enabling-features-for-your-repository/managing-github-actions-settings-for-a-repository
