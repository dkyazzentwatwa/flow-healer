# GitHub Actions Sandbox Best Practices (Research Stress 5)

This note condenses current GitHub primary-source guidance into a practical sandbox baseline for workflows that build code, process pull requests, or perform privileged follow-up automation. The recurring guidance across GitHub Docs is to minimize `GITHUB_TOKEN` permissions, keep untrusted code away from privileged triggers, prefer short-lived federated credentials, pin external actions immutably, and treat runners, caches, artifacts, logs, and chained workflows as part of the trust boundary.
...
- Run contributor-controlled code on `pull_request`, not `pull_request_target`. GitHub warns that `pull_request_target` can expose secrets, write permissions, or cache state if it is combined with untrusted code from a fork or branch.[3][4]
...
- Prefer GitHub-hosted runners for public repositories and workflows reachable by external contributors. GitHub-hosted runners start in a fresh environment for each job, while self-hosted runners do not inherently provide a clean instance per run.[4][5][6]
...
- Pin third-party actions to full commit SHAs. GitHub documents a full-length SHA as the only immutable action reference and exposes repository or organization settings that can require SHA pinning.[4][8]
...
- Keep logs, caches, and artifacts tight. GitHub recommends masking sensitive runtime values, setting explicit retention, and avoiding uploads that contain credentials, raw environment snapshots, or unnecessary debug data.[10][11]
- Use artifact attestations for high-value release outputs, then verify them where provenance matters. GitHub positions attestations as build provenance evidence, not as a substitute for verification policy.[12][13]
- Use repository or organization Actions policies to restrict the allowed action surface, require approval for fork PR workflows where appropriate, and enforce immutable pinning when governance matters.[8][14]
...
- Restrict which actions and reusable workflows can run through repository or organization policy when you need stronger supply-chain controls.[8][14]
- Require review or approval for fork PR workflows according to your threat model, and inspect workflow-file changes before approving runs from outside contributors.[3][8][14]
...
- Do generate attestations for release artifacts that consumers or admission policies are expected to verify.[12][13]
- Do use repository or organization policy to narrow the allowed action surface and tighten fork-workflow approval rules where appropriate.[8][14]
...
- Don't approve fork PR workflows casually, especially when the change set touches `.github/workflows/` or other execution paths.[14]
...
9. Are repository or organization policies enforcing the same action restrictions maintainers expect authors to follow manually?
...
13. GitHub Docs, "Use artifact attestations"  
    https://docs.github.com/en/actions/how-tos/secure-your-work/use-artifact-attestations/use-artifact-attestations
14. GitHub Docs, "Approving workflow runs from forks"  
