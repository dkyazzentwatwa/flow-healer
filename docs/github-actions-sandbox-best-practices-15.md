# GitHub Actions Sandbox Best Practices (Research Stress 15)

This note condenses current GitHub primary-source guidance into a practical sandbox baseline for repositories that run CI, release automation, or contributor-triggered workflows. The recurring guidance is consistent across GitHub Docs: keep `GITHUB_TOKEN` permissions minimal, avoid running untrusted code in privileged contexts, prefer short-lived federated credentials, pin third-party actions immutably, and treat runners, caches, logs, artifacts, and downstream workflows as part of the trust boundary.

## Current Best-Practice Summary

- Set explicit `permissions` on each workflow or job and keep them minimal. GitHub documents that once permissions are declared explicitly, unspecified scopes default to no access.[1][2]
- Run contributor-controlled code on `pull_request`, not `pull_request_target`. GitHub warns that combining `pull_request_target` with untrusted code can expose secrets, write tokens, or poison caches.[3][4]
- Treat `workflow_run` as privileged follow-up automation. GitHub notes that downstream workflows triggered this way can access secrets and write tokens even if the upstream workflow could not, so untrusted artifacts, outputs, or cache state must not flow into them without review.[3][4]
- Prefer GitHub-hosted runners for workflows influenced by forks or outside contributors. GitHub-hosted runners start with a fresh environment per job, while self-hosted runners do not inherently guarantee a clean instance for every run.[4][5][6]
- Prefer OIDC federation over long-lived cloud credentials. Grant `id-token: write` only to the specific job that needs cloud identity, and scope trust policies to the intended repository, ref, environment, or reusable workflow.[7]
- Pin third-party actions to a full commit SHA. GitHub documents a full-length SHA as the only immutable action reference.[4][8][9]
...
- Use artifact attestations for release outputs and other high-value build artifacts. GitHub positions attestations as a supply-chain control for establishing and verifying provenance, not as a blanket guarantee for every CI artifact.[12][13]
- Use repository, organization, or enterprise Actions policies to restrict allowed actions, require immutable pinning, and narrow who can approve or execute privileged automation.[8][9][14][15]
...
- Restrict which actions and reusable workflows can run through repository, organization, or enterprise policy when stronger supply-chain controls are needed.[8][9][14][15]
...
- Protect sensitive deployments with environments and approval controls when workflows can reach high-value secrets or production systems.[10][15]
...
- Do use repository, org, or enterprise policy to limit allowed actions and require immutable pinning where available.[8][9][14][15]
...
- Don't assume self-hosted runners are clean between jobs or safe for untrusted public PRs by default.[6][15]
...
### Privileged `workflow_run` Review

Treat `workflow_run` as a privilege boundary. If the downstream workflow has secrets or write scopes, review every artifact, output, cache input, or parameter it accepts from the upstream workflow as untrusted until explicitly validated.[3][4]
...
14. GitHub Docs, "Managing GitHub Actions settings for your organization"  
...
15. GitHub Docs, "Managing environments for deployment"  
