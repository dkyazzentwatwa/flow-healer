# GitHub Actions Sandbox Best Practices (Research Stress 13)

This note condenses current GitHub primary-source guidance into a practical sandbox baseline for CI, release automation, and contributor-triggered workflows. The most consistent themes across GitHub Docs are to keep `GITHUB_TOKEN` permissions minimal, avoid running untrusted code in privileged contexts, prefer short-lived federated credentials, pin third-party actions immutably, and treat runners, caches, artifacts, logs, and downstream workflows as part of the trust boundary.
