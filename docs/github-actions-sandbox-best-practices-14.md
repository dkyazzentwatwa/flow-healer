# GitHub Actions Sandbox Best Practices (Research Stress 14)

This note condenses current GitHub primary-source guidance into a practical sandbox baseline for repositories that run CI, deployment, or contributor-triggered automation. The consistent themes across GitHub Docs are to reduce token scope, keep untrusted code away from privileged triggers, prefer short-lived federated credentials, pin third-party actions immutably, and treat runners, artifacts, caches, and logs as part of the trust boundary.

## Current Best-Practice Summary

- Set explicit `permissions` on each workflow or job and keep them minimal. GitHub documents that once you specify any permission explicitly, all unspecified permissions default to `none`.[1][2]
- Run contributor-controlled code on `pull_request`, not `pull_request_target`. GitHub warns that `pull_request_target` can expose secrets, write permissions, or cache state if it is combined with untrusted code.[3][4]
- Treat `workflow_run` as privileged follow-up automation. GitHub notes that a downstream workflow triggered by `workflow_run` can access secrets and write tokens even when the upstream workflow could not, so artifacts, outputs, and caches from untrusted runs need explicit review before reuse.[3][4]
- Prefer GitHub-hosted runners for workloads influenced by forks or outside contributors. GitHub-hosted runners start in a fresh environment for each job, while self-hosted runners do not inherently provide a clean instance per run.[4][5][6]
- Prefer OIDC federation over stored long-lived cloud credentials. Grant `id-token: write` only to the job that actually needs cloud identity, then scope trust conditions to the intended repository, ref, environment, or reusable workflow context.[7]
- Pin third-party actions to full commit SHAs. GitHub documents a full-length SHA as the only immutable action reference and supports policies that enforce SHA pinning for actions and reusable workflows.[4][8][9]
...
- Keep logs, caches, and artifacts tight. Mask sensitive runtime values that are not stored as secrets, set explicit artifact retention, and avoid uploading credential-bearing or high-sensitivity files.[10][11]
- Use artifact attestations for release outputs and other high-value build artifacts when provenance matters. GitHub positions attestations as a supply-chain control that works best when consumers also verify them.[12][13]
- Use repository, organization, or enterprise Actions policies to narrow the allowed action surface, restrict reusable workflows, and enforce immutable pinning where governance matters.[8][9][14]
